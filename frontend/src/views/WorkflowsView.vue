...
async function executeWorkflow(w: WorkflowType): Promise<void> {
  try {
    ...
    if (res.success) showToast('success', `执行成功：${res.result}`)
    else showToast('error', `执行失败：${res.error ?? '未知错误'}`)
  } catch (e: unknown) {
...
<template>
  <div class="mx-auto max-w-6xl p-6">
    ...
    <div
      v-if="toast"
      :class="['mb-4 flex items-center gap-2 rounded-md border px-4 py-3 text-sm', toast.type === 'success' ? 'border-status-success/30 bg-status-success/10 text-status-success' : 'border-status-error/30 bg-status-error/10 text-status-error']"
    >
    ...
    <EmptyState v-else-if="workflows.length === 0" text="暂无工作流" description="点击右上角新建工作流开始编排" />
    <template v-else>
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div v-for="w in workflows" :key="w.id" class="card card-hover flex flex-col p-4">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <Badge :variant="w.is_enabled ? 'success' : 'default'" dot>{{ w.is_enabled ? '启用' : '停用' }}</Badge>
          </div>
          <p class="mb-3 min-h-[2.5rem] text-sm text-zinc-400">{{ w.description || '暂无描述' }}</p>
          <div class="mb-3 flex flex-wrap items-center gap-4 font-mono text-xs text-base-500">
            <span class="inline-flex items-center gap-1"><GitBranch class="h-3.5 w-3.5" />{{ w.nodes?.length ?? 0 }} 节点</span>
            <span class="inline-flex items-center gap-1"><Share2 class="h-3.5 w-3.5" />{{ w.edges?.length ?? 0 }} 边</span>
            <span class="inline-flex items-center gap-1"><Calendar class="h-3.5 w-3.5" />{{ formatDate(w.created_at) }}</span>
          </div>
          <div class="mt-auto flex items-center gap-1 border-t border-base-700 pt-3">
            <button class="btn-primary px-3 py-1.5 text-xs" @click="executeWorkflow(w)"><Play class="h-3.5 w-3.5" />执行</button>
            <button class="btn-ghost text-xs" @click="openHistory(w)"><History class="h-3.5 w-3.5" />历史</button>
            <button class="btn-ghost text-xs" @click="openEdit(w)"><Pencil class="h-3.5 w-3.5" />编辑</button>
            <button class="btn-ghost text-xs text-status-error hover:bg-status-error/10" @click="deleteId = w.id"><Trash2 class="h-3.5 w-3.5" />删除</button>
          </div>
        </div>
      </div>
    ...
    <!-- 新建/编辑弹窗 -->
    <BaseModal :model-value="formOpen" :title="isEdit ? '编辑工作流' : '新建工作流'" width="max-w-2xl" @update:model-value="formOpen = $event">
      <div class="space-y-4">
        <div>
    ...
    <!-- 历史弹窗 -->
    <BaseModal :model-value="historyOpen" :title="`执行历史 - ${historyName}`" width="max-w-2xl" @update:model-value="historyOpen = $event">
      ...
      <div v-else class="max-h-[60vh] space-y-3 overflow-y-auto pr-1">
        <div v-for="ex in historyList" :key="ex.id" class="rounded-md border border-base-700 bg-base-900 p-3">
          <div class="mb-1 flex items-center justify-between">
            <Badge :variant="statusVariant(ex.status)" dot>{{ ex.status }}</Badge>
            <span class="font-mono text-xs text-base-500">#{{ ex.id }}</span>
          </div>
          <p v-if="ex.result" class="mb-1 break-all text-xs text-zinc-300"><span class="text-base-500">结果：</span>{{ ex.result }}</p>
          <p v-if="ex.error" class="mb-1 break-all text-xs text-status-error"><span class="text-base-500">错误：</span>{{ ex.error }}</p>
          <div class="font-mono text-xs text-base-500">
            开始：{{ formatDate(ex.started_at) }}<span v-if="ex.finished_at"> · 完成：{{ formatDate(ex.finished_at) }}</span>
          </div>
        </div>
      </div>
      <template #footer>
    ...
    <!-- 删除确认 -->
    <ConfirmDialog :model-value="deleteId !== null" title="删除工作流" text="确定要删除此工作流吗？此操作不可撤销。" @update:model-value="deleteId = $event ? deleteId : null" @confirm="doDelete" />
  </div>
</template>
