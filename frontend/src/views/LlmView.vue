...
function openCreateModal(): void {
  ...
  Object.assign(form, { name: '', api_key: '', api_base: '', model_name: '', max_tokens: 4096, temperature: 0.7 })
...
function openEditModal(config: LlmConfig): void {
  ...
  Object.assign(form, {
    name: config.name, api_key: config.api_key, api_base: config.api_base,
...
async function handleSubmit(): Promise<void> {
  ...
  try {
    if (editingConfig.value) {
      await llmApi.updateConfig(editingConfig.value.id, { ...form })
      showMessage('success', '配置已更新')
    } else {
      await llmApi.createConfig({ ...form })
...
<template>
  <div class="mx-auto max-w-6xl p-6">
    ...
    <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2">
      <div v-for="config in configs" :key="config.id" class="card card-hover flex flex-col p-5">
        ...
        <div class="mb-4 space-y-2">
          ...
          <div class="flex items-center gap-4">
            <span class="flex items-center gap-1.5">
              <Hash class="h-3.5 w-3.5 text-base-500" /><span class="text-label">tokens</span>
              <span class="font-mono text-xs text-zinc-300">{{ config.max_tokens }}</span>
            </span>
            <span class="flex items-center gap-1.5">
              <Thermometer class="h-3.5 w-3.5 text-base-500" /><span class="text-label">temp</span>
        ...
        <div class="mt-auto flex items-center gap-2 border-t border-base-700 pt-3">
          <button v-if="!config.is_active" class="btn-primary px-3 py-1.5 text-xs" @click="handleActivate(config)">
            <Zap class="h-3.5 w-3.5" /> 激活
          </button>
    ...
    <BaseModal :model-value="showFormModal" :title="editingConfig ? '编辑配置' : '新建配置'" width="max-w-xl" @update:model-value="showFormModal = $event">
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input v-model.number="form.temperature" type="range" min="0" max="2" step="0.1" class="h-2 w-full cursor-pointer appearance-none rounded-full bg-base-700 accent-amber" />
          </div>
        </div>
      </form>
      <template #footer>
        ...
        <button class="btn-primary" :disabled="submitting" @click="handleSubmit">
          <Loader2 v-if="submitting" class="h-4 w-4 animate-spin" /> {{ editingConfig ? '保存' : '创建' }}
        </button>
      </template>
    </BaseModal>
    ...
    <BaseModal v-model="showModelsModal" title="可用模型列表" width="max-w-lg">
      ...
      <div v-else class="max-h-80 space-y-1.5 overflow-y-auto">
        <div v-for="model in models" :key="model" class="rounded-md border border-base-700 bg-base-900 px-3 py-2 font-mono text-sm text-cyan">
          {{ model }}
        </div>
      </div>
      <template #footer>
